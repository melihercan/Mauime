...
public class KnownDefectTests
{
    ...
    public async Task DEFECT_XamarinHost_claims_IHost_but_throws_from_StartAsync()
    {
        ...
        var host = Build("Mauime.Tests.TestAssets.EnvMissing");
    ...
    public void DEFECT_XamarinHost_Dispose_leaves_its_service_provider_undisposed()
    {
        ...
        var builder = XamarinHostBuilder.CreateDefault(Options("Mauime.Tests.TestAssets.EnvMissing"));
    ...
    public void DEFECT_Hosting_swallows_every_error_while_reading_the_environment()
    {
        ...
        XamarinHostBuilder.CreateDefault(Options("Mauime.Tests.TestAssets.EnvNumeric"))
    ...
    public void DEFECT_Hosting_turns_a_json_null_environment_into_a_null_environment()
    {
        ...
        var builder = XamarinHostBuilder.CreateDefault(Options("Mauime.Tests.TestAssets.EnvNull"));
    ...
    public void DEFECT_Hosting_lets_a_malformed_appsettings_escape_the_catch_it_wrote()
    {
        ...
        var create = () => XamarinHostBuilder.CreateDefault(Options("Mauime.Tests.TestAssets.Malformed"));
    ...
    // ---- Xamarinme.Nfc ------------------------------------------------------------------------
    //
    // legacy/Nfc is not in the solution and has no runtime coverage: MSBuild.Sdk.Extras needs
    // desktop msbuild.exe for MonoAndroid10.0, Xamarin.iOS10, uap10.0.19041 and Xamarin.Mac20, and
    // the netstandard2.0 slice does not compile even under it. These two pins are textual for that
    // reason, and become real tests in the phase that ports the library.

    [Fact]
    public void DEFECT_Nfc_netstandard_slice_does_not_compile()
    {
        // CrossNfc.CreateNfc is meant to return null on the platform-neutral slice and a real Nfc
        // everywhere else. NETSTANDARD2_0 is not defined for that slice under this toolchain, so
        // the #else branch is compiled and `new Nfc()` names a type that exists on no platform
        // the slice can see:
        //     legacy/Nfc/CrossNfc.cs(30,24): error CS0246: The type or namespace name 'Nfc'
        //     could not be found
        var source = LegacySource.Read("Nfc", "CrossNfc.cs");

        source.Should().Contain("#if NETSTANDARD2_0");
        source.Should().Contain("return new Nfc();");
    }

    [Fact]
    public void DEFECT_Nfc_Pcsc_carries_a_dead_if_false_block_with_a_second_constructor()
    {
        // Roughly 180 lines of superseded PC/SC code, including a whole second Pcsc() constructor,
        // left behind an #if false. It is not conditional compilation for a platform; it is a
        // commit that was never finished.
        var lines = Lines(LegacySource.Read(Path.Combine("Nfc", "Shared"), "Pcsc.cs"));

        var start = lines.IndexOf("#if false");
        var end = lines.LastIndexOf("#endif");

        start.Should().BeGreaterThan(-1);
        (end - start).Should().BeGreaterThan(150, "the dead region is about 180 lines");
        lines.Count(l => l == "public Pcsc()").Should().Be(2, "one of them is inside the dead region");
    }
...
internal static class LegacySource
{
    ...
    {
        var path = Path.Combine(TestAssemblies.RepositoryRoot, "legacy", projectDirectory, fileName);
