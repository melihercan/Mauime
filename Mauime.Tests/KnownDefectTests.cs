...
public class KnownDefectTests
{
    ...
    public async Task FIXED_Nfc_has_a_platform_neutral_slice_that_compiles_and_loads()
    {
        ...
        // build and had no test coverage at all. Mauime.Nfc's net10.0 slice is the same role — the
    ...
    public void FIXED_Nfc_carries_no_dead_conditional_blocks()
    {
        ...
        foreach (var file in Directory.GetFiles(
                     Path.Combine(TestAssemblies.RepositoryRoot, "Mauime.Nfc"), "*.cs",
    ...
    public void FIXED_The_platform_implementations_are_internal()
    {
        ...
        // implementation directly. Mauime exposes INfc and the registration extension; every
    ...
    public void LIMITATION_The_Android_and_iOS_implementations_have_no_behavioural_coverage()
    {
        ...
        // Mauime.Nfc, which is the one with no implementation, so nothing here exercises
    ...
    private static ServiceProvider NfcServices() =>
        MauiApp.CreateBuilder(useDefaults: false).UseMauimeNfc().Services.BuildServiceProvider();

    private static string PlatformSource(string platform, string fileName) =>
        File.ReadAllText(Path.Combine(
            TestAssemblies.RepositoryRoot, "Mauime.Nfc", "Platforms", platform, fileName));
}
