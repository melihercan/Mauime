...
/// <summary>
/// <see cref="MauimeConfigurationExtensions"/>, through its embedded-resource path.
///
...
{
    private const string Prefix = "Mauime.Tests.TestAssets";

    private static Assembly Assembly => typeof(ConfigurationTests).Assembly;

    private static IConfigurationRoot Build(
        string fileName = MauimeConfigurationExtensions.DefaultFileName,
    ...
    {
        MauimeConfigurationExtensions.EnvironmentFileName(fileName, environment).Should().Be(expected);
    }
