...
/// <summary>
/// That every Mauime library really is built for every MAUI target framework, and that all of its
...
public class MultiTargetingTests
{
    ...
    {
        // Directory.Build.targets writes Mauime.ReferencePaths.txt beside each assembly, and the
        ...
        foreach (var framework in TestAssemblies.BuiltTargetFrameworks(assemblyName))
        {
            ...
            var referencePaths = Path.Combine(Path.GetDirectoryName(dll)!, "Mauime.ReferencePaths.txt");
