...
///
/// Mauime ships under new package IDs, so there are no existing consumers and this is *not* an
...
public class PublicApiSurfaceTests
{
    ...
    public void Public_surface_matches_the_approved_baseline()
    {
        ...
        var approvedPath = Path.Combine(TestAssemblies.RepositoryRoot, "Mauime.Tests", "PublicApi.approved.txt");
        ...
        Normalize(received).Should().Be(
            ...
            + "copy '{0}' over 'Mauime.Tests/PublicApi.approved.txt' in the same commit.",
            receivedPath);
    }
