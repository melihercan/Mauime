...
/// <summary>
/// <see cref="MauimeHostingExtensions"/>, which is all that survived of Xamarinme.Hosting.
///
...
public class HostingTests
{
    ...
    [Fact]
    public void UseMauimeHosting_sets_the_environment_name()
    {
        var environment = Environment(Builder().UseMauimeHosting(Environments.Development));
    ...
    {
        Environment(Builder().UseMauimeHosting("QA")).EnvironmentName.Should().Be("QA");
    }

    [Fact]
    public void UseMauimeHostingFromConfiguration_reads_the_name_from_configuration()
    {
        var builder = Builder();
        builder.Configuration.AddInMemoryCollection(
            [new(MauimeHostingExtensions.DefaultConfigurationKey, "Development")]);

        Environment(builder.UseMauimeHostingFromConfiguration()).EnvironmentName.Should().Be("Development");
    }
    ...
    public void Configuration_added_after_the_call_is_still_seen()
    {
        ...
        var builder = Builder().UseMauimeHostingFromConfiguration();

        builder.Configuration.AddInMemoryCollection(
            [new(MauimeHostingExtensions.DefaultConfigurationKey, "Staging")]);
    ...
    public void A_custom_configuration_key_is_honoured()
    {
        ...
        Environment(builder.UseMauimeHostingFromConfiguration("Deployment:Ring"))
    ...
    {
        Environment(Builder().UseMauimeHostingFromConfiguration())
    ...
    public void An_empty_value_leaves_the_platform_default_alone()
    {
        var builder = Builder();
        builder.Configuration.AddInMemoryCollection(
            [new(MauimeHostingExtensions.DefaultConfigurationKey, "")]);
    ...
    public void The_environment_reads_from_an_embedded_appsettings_file()
    {
        ...
        builder.Configuration.AddEmbeddedResourceJson(
            typeof(HostingTests).Assembly, "Mauime.Tests.TestAssets", "environment.json");

        Environment(builder.UseMauimeHostingFromConfiguration()).EnvironmentName.Should().Be("Staging");
    }

    [Fact]
    public void The_environment_is_resolved_once()
    {
        var builder = Builder().UseMauimeHosting("Development");
        var provider = builder.Services.BuildServiceProvider();
    ...
    public void Both_extensions_return_the_builder_so_calls_can_be_chained()
    {
        ...
        one.UseMauimeHosting("Development").Should().BeSameAs(one);
        two.UseMauimeHostingFromConfiguration().Should().BeSameAs(two);
    }
    ...
    {
        { "null builder", () => ((MauiAppBuilder)null!).UseMauimeHosting("Development") },
        { "null environment name", () => MauiApp.CreateBuilder(false).UseMauimeHosting(null!) },
        { "empty environment name", () => MauiApp.CreateBuilder(false).UseMauimeHosting("") },
        { "null builder, from configuration", () => ((MauiAppBuilder)null!).UseMauimeHostingFromConfiguration() },
        { "empty key", () => MauiApp.CreateBuilder(false).UseMauimeHostingFromConfiguration("") },
    };
}
