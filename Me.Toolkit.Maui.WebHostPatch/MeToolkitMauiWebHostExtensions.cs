...
public static class MeToolkitMauiWebHostExtensions
{
    /// <summary>
    /// Registers <see cref="IMeToolkitMauiWebHost"/> as a singleton. The server is not started; call
    /// <see cref="IMeToolkitMauiWebHost.StartAsync"/> when the app is ready for it.
    /// </summary>
    ...
    /// </remarks>
    public static MauiAppBuilder UseMeToolkitMauiWebHost(
        this MauiAppBuilder builder,
        Action<MeToolkitMauiWebHostOptions>? configure = null)
    {
        ArgumentNullException.ThrowIfNull(builder);

        var options = new MeToolkitMauiWebHostOptions();
        configure?.Invoke(options);

        builder.Services.AddSingleton<IMeToolkitMauiWebHost>(_ => new MeToolkitMauiWebHost(options));
